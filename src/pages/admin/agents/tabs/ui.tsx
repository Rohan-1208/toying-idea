...
const STATUS_TONE: Record<string, string> = {
  ...
  ok: "bg-ink/5 text-ink",
};

export function StatusPill({ status, className }: { status?: string | null; className?: string }) {
  const s = status ?? "—";
  return (
    <span className={cx("inline-flex items-center rounded-full px-2.5 py-0.5 text-[11px] font-medium whitespace-nowrap", STATUS_TONE[s] ?? "bg-ink/5 text-ink", className)}>
      {s.replaceAll("_", " ")}
    </span>
  );
}
...
export function RiskBadge({ risk }: { risk?: Risk }) {
  if (!risk) return null;
  const tone = risk === "action" ? "bg-clay/15 text-ink border-clay/40" : risk === "draft" ? "bg-ti-gold/25 text-ink border-ti-gold/60" : "bg-ti-sky/20 text-ink border-ti-sky/50";
...
export function Panel({ title, subtitle, actions, children, className }: { title?: ReactNode; subtitle?: ReactNode; actions?: ReactNode; children: ReactNode; className?: string }) {
  return (
    <section className={cx("rounded-3xl border border-ink/10 bg-cream-50 ti-ring", className)}>
      {title || actions ? (
        <header className="flex flex-wrap items-start justify-between gap-3 px-5 pt-5">
          <div className="grid gap-0.5">
            {title ? <h2 className="font-display text-lg tracking-tight">{title}</h2> : null}
...
export function Stat({ label, value, hint, tone }: { label: string; value: ReactNode; hint?: ReactNode; tone?: "warn" | "bad" | "good" }) {
  return (
    <div className="rounded-2xl border border-ink/10 bg-cream-50 px-4 py-3 grid gap-1">
      ...
      {hint ? <div className="text-[11px] text-ink/50">{hint}</div> : null}
    </div>
  );
}
...
export function SmallButton({ children, onClick, variant = "ghost", disabled, type = "button", title }: {
  ...
  const v =
    variant === "primary"
      ? "bg-clay text-cream-50 hover:brightness-95"
      : variant === "dark"
...
export const inputCls = "h-10 w-full rounded-[var(--radius-sm)] border border-ink/10 bg-cream px-3 text-sm focus:outline-none focus:ring-2 focus:ring-ti-sky/60";
export const textareaCls = "w-full rounded-[var(--radius-sm)] border border-ink/10 bg-cream px-3 py-2 text-sm leading-relaxed focus:outline-none focus:ring-2 focus:ring-ti-sky/60";

export function Empty({ children }: { children: ReactNode }) {
  return <div className="rounded-2xl border border-dashed border-ink/10 px-4 py-8 text-center text-sm text-ink/50">{children}</div>;
}

export function Json({ value }: { value: unknown }) {
  return (
    <pre className="max-h-64 overflow-auto rounded-[var(--radius-sm)] bg-ink/[0.04] p-3 text-[11px] leading-relaxed text-ink/90 whitespace-pre-wrap break-words">
...
export function AgentGlyph({ agent, className }: { agent: string; className?: string }) {
  return (
    <span className={cx("grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-ink text-ti-gold text-base", className)} aria-hidden>
      {AGENT_GLYPH[agent] ?? "●"}
    </span>
  );
}
