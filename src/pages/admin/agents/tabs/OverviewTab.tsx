...
import { timeAgo, usd } from "../api";
import { AgentGlyph, Empty, Panel, SmallButton, Stat, StatusPill, cx } from "./ui";

export function OverviewTab({ ctx }: { ctx: DashboardCtx }) {
  const o = ctx.overview;
  if (!o) return <div className="text-sm text-ink/50">Loading…</div>;
    ...
    <div className="grid gap-5">
      ...
      <Panel title="Your agents" subtitle="Click an agent to give it a task. Configure models, autonomy and instructions in the Agents tab.">
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {o.agents.map((a) => (
            <div key={a.key} className={cx("grid gap-3 rounded-2xl border border-ink/10 bg-cream-50 p-4", !a.enabled && "opacity-60")}>
              <div className="flex items-start gap-3">
                ...
                <div className="grid min-w-0 gap-0.5">
                  ...
                  <div className="text-xs text-ink/50 line-clamp-2">{a.description}</div>
                </div>
              </div>
              <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-[11px] text-ink/50">
                <span>{a.model}</span>
        ...
        <Panel title="Latest activity" actions={<SmallButton onClick={() => ctx.goTo("activity")}>All activity</SmallButton>}>
          {o.recent_runs.length === 0 ? (
            ...
            <ul className="divide-y divide-border">
              {o.recent_runs.map((r) => (
                <li key={r.id}>
                  <button type="button" onClick={() => ctx.openRun(r.id)} className="flex w-full items-start gap-3 py-3 text-left hover:bg-ink/5/60 rounded-lg px-2 -mx-2">
                    ...
                    <div className="grid min-w-0 flex-1 gap-0.5">
                      ...
                      <div className="truncate text-xs text-ink/50">{r.output || r.error || r.task}</div>
                    </div>
                    <div className="text-[11px] text-ink/50 whitespace-nowrap">{timeAgo(r.created_at)}</div>
                  </button>
                </li>
              ))}
            </ul>
...
function SetupRow({ ok, label, hint, optional }: { ok: boolean; label: string; hint: string; optional?: boolean }) {
  return (
    <li className="flex items-start gap-3">
      <span className={cx("mt-0.5 grid h-5 w-5 shrink-0 place-items-center rounded-full text-[11px] font-bold", ok ? "bg-emerald-600 text-white" : optional ? "bg-stone-200 text-stone-600" : "bg-amber-400 text-ink")}>
        {ok ? "✓" : optional ? "–" : "!"}
      </span>
      <div className="grid">
