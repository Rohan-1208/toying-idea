...
import { formatDateTime, usd, useApi, type Run } from "../api";
import { AgentGlyph, Empty, StatusPill, inputCls } from "./ui";
      ...
      {runs === null ? (
        ...
        <div className="overflow-x-auto rounded-3xl border border-ink/10 bg-cream-50">
          <table className="w-full min-w-[760px] text-sm">
            ...
            <tbody>
              {runs.map((r) => (
                <tr key={r.id} className="cursor-pointer border-b border-ink/10/60 last:border-0 hover:bg-ink/5/60" onClick={() => ctx.openRun(r.id)}>
                  <td className="px-4 py-3 whitespace-nowrap text-xs text-ink/50">{formatDateTime(r.created_at)}</td>
                  ...
                  <td className="px-4 py-3 text-right text-xs tabular-nums text-ink/50">{usd(r.usage?.cost_usd)}</td>
                </tr>
              ))}
            </tbody>
          </table>
