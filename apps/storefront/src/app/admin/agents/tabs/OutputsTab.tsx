...
import { api, formatDateTime, timeAgo, useApi, type ContentItem, type CustomerMessage, type Insight } from "@/app/admin/agents/api";
import { Empty, Json, SmallButton, StatusPill, cx, textareaCls } from "@/app/admin/agents/tabs/ui";
    ...
    <div className="grid gap-4">
      <div className="flex flex-wrap gap-1 rounded-full border border-border bg-surface p-1 w-fit">
        {KINDS.map((k) => (
          <button
            ...
            className={cx("rounded-full px-3 py-1.5 text-xs font-medium", kind === k.key ? "bg-ti-cocoa text-ti-cream" : "text-muted hover:text-ti-cocoa")}
          >
            {k.label}
          </button>
        ))}
      ...
      {items === null ? (
        ...
        <div className="grid gap-3">
          {items.map((a) => (
            <details key={a.id} className="rounded-[var(--radius-lg)] border border-border bg-surface p-4">
              <summary className="flex cursor-pointer flex-wrap items-center gap-2 text-sm">
                ...
                <span className="text-[11px] text-muted">{timeAgo(String(a.created_at))}</span>
              </summary>
              <div className="mt-3">
...
function InsightCard({ i, ctx }: { i: Insight; ctx: DashboardCtx }) {
  return (
    <article className="grid gap-3 rounded-[var(--radius-lg)] border border-border bg-surface p-5">
      <header className="flex flex-wrap items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          <h3 className="font-[var(--font-ti-display)] text-lg tracking-tight">{i.title}</h3>
          <StatusPill status={i.kind} />
        </div>
        <span className="text-[11px] text-muted">{formatDateTime(i.created_at)}</span>
      </header>
      ...
      {i.recommendations?.length ? (
        <ul className="grid gap-2">
          {i.recommendations.map((r, idx) => (
            <li key={idx} className="flex flex-wrap items-start justify-between gap-3 rounded-[var(--radius-md)] bg-ti-cream px-3 py-2">
...
function MessageCard({ m, onChanged }: { m: CustomerMessage; onChanged: () => Promise<void> }) {
  ...
  return (
    <article className="grid gap-2 rounded-[var(--radius-lg)] border border-border bg-surface p-4">
      ...
      {m.subject ? <div className="text-xs text-muted">Subject: {m.subject}</div> : null}
      {editable ? (
...
function ContentCard({ c, onChanged }: { c: ContentItem; onChanged: () => Promise<void> }) {
  ...
  return (
    <article className="grid content-start gap-2 rounded-[var(--radius-lg)] border border-border bg-surface p-4">
      ...
      <div className="text-xs text-ti-cocoa/70">{c.hashtags?.join(" ")}</div>
      <details className="text-xs">
        <summary className="cursor-pointer text-muted">Visual brief{c.reel_script ? " & reel script" : ""}</summary>
        <div className="mt-2 grid gap-2 whitespace-pre-wrap">
          <div>{c.visual_brief}</div>
          {c.reel_script ? <div className="rounded bg-ti-cream p-2">{c.reel_script}</div> : null}
        </div>
      </details>
