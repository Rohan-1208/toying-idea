...
import { api, useApi, type AgentConfig } from "@/app/admin/agents/api";
import { AgentGlyph, Field, Panel, RiskBadge, SmallButton, Toggle, cx, inputCls, textareaCls } from "@/app/admin/agents/tabs/ui";
        ...
        {data.agents.map((a) => (
          <button
            ...
            className={cx(
              ...
              a.key === agent.key ? "border-ti-orange bg-surface ti-ring" : "border-border hover:bg-surface",
            )}
          >
            <AgentGlyph agent={a.key} className="h-8 w-8 text-sm" />
            <div className="grid min-w-0">
              <span className="truncate text-sm font-medium">{a.name}</span>
              <span className="truncate text-[11px] text-muted">{a.enabled ? a.model : "disabled"}</span>
            </div>
          </button>
        ))}
      </nav>
  ...
  return (
    <div className="grid gap-5">
            ...
            return (
              <div key={g.risk} className="grid gap-2">
                ...
                <ul className="grid gap-2 md:grid-cols-2">
                  {list.map((t) => (
                    <li key={t.name} className="grid gap-1 rounded-[var(--radius-md)] border border-border bg-ti-cream/50 p-3">
      ...
      <details className="rounded-[var(--radius-lg)] border border-border bg-surface p-5">
        ...
        <pre className="mt-3 whitespace-pre-wrap text-xs leading-relaxed text-muted">{agent.system_prompt}</pre>
      </details>
