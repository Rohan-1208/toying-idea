...
import { api, formatDateTime, timeAgo, useApi, type Workflow } from "@/app/admin/agents/api";
import { AgentGlyph, Empty, Field, Panel, SmallButton, Toggle, cx, inputCls, textareaCls } from "@/app/admin/agents/tabs/ui";
      ...
      {data.workflows.length === 0 ? (
        ...
        <div className="grid gap-3">
          {data.workflows.map((w) => (
            <div key={w.id} className={cx("grid gap-3 rounded-[var(--radius-lg)] border border-border bg-surface p-4 md:grid-cols-[auto_1fr_auto] md:items-center", !w.enabled && "opacity-70")}>
              ...
              <div className="grid min-w-0 gap-1">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <span className={cx("rounded-full px-2 py-0.5 text-[11px] font-medium", w.trigger.type === "event" ? "bg-ti-sky/25" : w.trigger.type === "schedule" ? "bg-ti-gold/30" : "bg-surface-2")}>
                    {w.trigger_label}
                  </span>
                ...
                {w.description ? <div className="text-xs text-muted">{w.description}</div> : null}
...
function WorkflowEditor({ initial, meta, onClose, onSaved }: { initial: Partial<Workflow>; meta: WfResponse; onClose: () => void; onSaved: () => Promise<void> }) {
  ...
  return (
    <Panel title={w.id ? `Edit “${initial.name}”` : "New workflow"} actions={<SmallButton onClick={onClose}>Close</SmallButton>} className="border-ti-orange/50">
